...
REVIEWED = {
    ...
    # P1-02/W — صارا يُنتجان مستنًدا، فصار لهما قراٌر مكتوب لا سكوت.
    "REQOT": "HRMS-PR-026",        # OD-018 — «تكليف واعتماد عمل إضافي»
    "ADMDED": "HRMS-PR-021",       # OD-008 — «قرار خصم» بالحرف، وقالبه الوحيد
    # **وإغفاٌل يكشفه اسُم المستند**: ``OD-021`` اسمُه «إشعار تحديث بيانات
    ...
    "REQALLOW": "HRMS-PR-020",     # OD-005 — «قرار بدل أو مكافأة» — قرار المالك 2026-09-18
    # **قراُر المالك (2026-09-12): الغلاُف الداخلي للأربعة.** وقالبُه واحٌد
    # — HRMS-PR-034 ← OD-013 — هو نفسه الذي يخدم تجديَد الإقامة. وكانت
    # قوالبُها تشير إلى «محضر تحقيق» و«قرار إنذار»، وذلك يُختَم على الأثر.
    "REQWP": "HRMS-PR-034",        # OD-013 — غلاف متابعة معاملة حكومية
    "REQPASS": "HRMS-PR-034",      # OD-013
    "REQCID": "HRMS-PR-034",       # OD-013
}
...
REGISTRY_IDENTIFIED = {
    # **وقالٌب خاطٌئ يُنزَع ال يُتبَع**: ``REQDED`` كان يشير إلى
    # ``HRMS-PR-033`` وهو ``RPT-005`` (تقرير) ال ``OD-024``. فنُزع القالُب
    # وبقيت الهويُّة من السجلّ: ``WF-012`` يعلن ``OD-024`` وحده — وهو
    # «قرار اعتراض راتب / **خصم**» باسمه، و``REQPAY`` شقيقُه يُنتجه.
    "REQDED": "WF-012 → OD-024 — قرار اعتراض خصم، وقالبه كان يشير إلى تقرير.",
    ...
    # وما كان يُعَدّ «مستنداٍت تُصاغ وتُعتمد» لم يكن ينقصه إلا أن يُقال
    # «أصدِرْه»: مساُر كلٍّ منها يعلن مستنًدا واحًدا، فالهويّة محسومة.
    "REQPER": "WF-003 → OD-018 — إيصال إذن.",
    "REQEXIT": "WF-003 → OD-018 — إيصال مغادرة مبكرة.",
    "REQLATE": "WF-004 → OD-018 — إيصال تبرير تأخير.",
    "REQATT": "WF-004 → OD-018 — إيصال تصحيح حضور.",
    "REQEXP": "WF-010 → OD-023 — إيصال مصروفات.",
    "REQTRN": "WF-027 → OD-020 — طلب/نتيجة تدريب.",
    # وهذه أُصدرت بعد أن صار الأرشيف يحفظ السرّية.
    "REQDATA": "WF-007 → OD-021 — إشعار تحديث بيانات (سرّي).",
    "REQCONTACT": "WF-007 → OD-021 — إشعار تحديث اتصال (سرّي).",
    "REQPAY": "WF-011 → OD-024 — قرار اعتراض راتب (سرّي).",
    "REQVIO": "WF-015 → OD-009 — إقرار/ردّ الموظف (سرّي).",
    "REQWARN": "WF-015 → OD-009 — إقرار/ردّ على إنذار (سرّي).",
    "REQGRV": "WF-016 → OD-010 — قرار نتيجة تظلّم (سرّي).",
}
...
#: أنواع **تُنتج مستنًدا** ولا يقطع السجلّ في خريطتها — **وكلّها محسومة
#: بقرار المالك**. تبقى هنا لأن الجسر لا يستطيع التحقّق منها، لا لأنها
#: تنتظر جواًبا.
#:
#: وقيمة إبقائها مثبَّتة بالاسم أن **نوًعا جديًدا** يفلت من الفحص يسقط
#: الاختبار يوم يُضاف — فلا تنمو قائمة ما لا يُفحَص صامتًة.
KNOWN_UNVERIFIABLE = {
    # مساره WF-029 لا يعلن مستنًدا، فلا شيء يُقارَن به. **وقرار المالك:
    # يبقى كما هو** — قالبه HRMS-PR-036 (→ OD-013، صنف المعاملات
    # الحكومية) موافق لموضوعه: مهمة عمل خارجية. فالنقص في إعلان المسار
    # لا في الخريطة، ويُحسَم يوم يُستكمل السجلّ.
    "REQMIS",
...
def test_every_template_question_is_now_decided():
    """**وآخر أسئلة القوالب أُغلق.**

    كانت ثلاثة تُنتج مستنًدا ولا يقطع السجلّ في خريطتها:

    * ``ADMLIC`` — بُني له قالب الشركة (HRMS-PR-043 → OD-013)
    * ``REQWLOC`` — رُبط بـHRMS-PR-017 → OD-005، موافًقا لما يعلنه مساره
    * ``REQMIS`` — **يبقى كما هو** بقرار المالك
