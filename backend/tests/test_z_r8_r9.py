...
def test_sec02_twofa_required_for_sensitive_roles(client):
    ...
    # قرار المالك (2026-09-24): صاحب الشركات وحده؛ مدير الشركة اختياري بمحض إرادته.
    assert TWOFA_REQUIRED_ROLES == {"company_owner"}
    for opt in ("super_admin", "company_manager", "hr", "delegate"):
...
def test_qa10_a_leave_without_travel_completes_without_delegate(client):
    ...
    # ومرحلة الاستلام أُضيفت لاحًقا بقرار المالك: المستند يصدر ويُقال أين
    # يُستلَم. فصارت ثلاًثا — والمقصود هنا أن **المندوب** ليس فيها.
