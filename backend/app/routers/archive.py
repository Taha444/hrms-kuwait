...
async def replace_custom_document(
    ...
    # الاستبدال على النسخة **الحالية** وحدها: على نسخةٍ تاريخية كان يُنتج نسخةً ثانيةً «حالية»
    # برقم إصدارٍ مكرَّر، والحاليةُ الحقيقية تبقى حالية — فللمستند نسختان تدّعيان أنهما الأحدث.
    if not old.is_current:
        raise HTTPException(status_code=409, detail=(
            "هذه نسخة قديمة من المستند — استبدل النسخة الحالية (الأحدث) لا نسخةً من التاريخ"))
