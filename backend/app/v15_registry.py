...
CANONICAL_WORKFLOWS: dict[str, dict] = {
    ...
    "WF-002": {"name_ar": "إجازة سفر", "name_en": "Travel Leave", "od": ["OD-011", "OD-013"]},  # OD-012 رُفعت — قرار المالك 2026-09-18: التنبيه يكفي
...
LEGACY_REQUEST_ALIASES: dict[str, dict] = {
    ...
    # STR-07 — أكواد لها نموذج (form_schema) بلا صف RequestType. بلا ربطها
    # يُقبل الطلب ثم يعجز النظام عن حلّ نوعه فيقف بلا مسار.
    "REQLV": {"canonical": "WF-001", "note": "كود النموذج — يُحَل إلى نوع الإجازة"},
...
LEGACY_PRN_ALIASES: dict[str, str] = {
    ...
    # تجديد مستند/ترخيص شركة — صنف المعاملات الحكومية، كإخوته في فئة
    # البذرة نفسها (PR-034..PR-037). مشتقّ من الفئة لا من الاسم.
    "HRMS-PR-043": "OD-013", "PRN-043": "OD-013",
}
...
#: **مخرٌج يُولَّد في مرحلٍة بعينها، ال مرًة واحدة مع الطلب.**
#:
#: الهويُّة تُحَل لكل **نوع**: مساٌر يعلن مستنًدا واحًدا فهو، ويعلن عدًدا
#: فالقالب يرّجح. وذلك يكفي نوًعا يُنتج ورقًة واحدة — وال يكفي إجازًة
#: تُعتمد فتُنتج **قراَرها**، ثم يُجهَّز لها إذُن مغادرة فتُنتج **غالَف
#: متابعٍة** ثانًيا.
#:
#: والسفُر ليس نوًعا آخر: ``leave`` يبقى ``WF-001`` ويُرقَّى إلى ``WF-002``
#: بالحمولة (``_PAYLOAD_PROMOTIONS``)، ومرحلُة المندوب شرطيٌة بـ
#: ``travel_required``. **فال شيَء يُحيل إلى ``WF-002`` بكوده**، ومخرجاتُه
#: اإلضافية غيُر قابلٍة للبلوغ ما لم تُنسَب إلى مرحلٍة.
#:
#: فتُعلَن هنا: (كوُد النوع، نوُع المرحلة) ← المستند. ويقرؤها موضعان —
#: ``produced_outputs`` لتُحتسب، والمولُّد ليُقيَّد بها.
STAGE_EXTRA_OUTPUTS: dict[tuple[str, str], str] = {
    ("leave", "delegate_exit"): "OD-013",
}


#: رمُز الطلب ← **فئُة الجهة** في مفردات ``portals.CATEGORY_LABELS``.
#:
#: **وكان الغلاُف يطابق ``category == rt.code`` — وهو بحٌث لا يُصيب أبدًا.**
#: فـ``create_portal`` يرفض كلَّ فئٍة خارج ``CATEGORY_LABELS`` («فئة غير
#: معروفة»)، والشاشُة لا تعرض غيرها في القائمة. فلا سبيَل — بالشاشة ولا
#: بالـAPI — لإنشاء صٍّف فئتُه ``leave`` أو ``REQWP``. فسطُر «الجهة
#: الحكومية» كان **يُسقَط دائمًا**، والحقُل الذي يشترطه السجلّ
#: (``government_entity``) لا يُملأ أبدًا.
#:
#: **ويُقرأ اسُم الجهة من المفردات لا من اسم الرابط.** فـ``name_ar`` اسُم
#: *موقٍع* («سهل»، «PACI») لا اسُم جهة، و``db.scalar`` بلا ترتيٍب يأخذ
#: صًّفا يختاره الحظّ من عدّة روابط في الفئة. فيُكتَب المسمّى المعلَن
#: («العمل والعمالة») — ولا يحتاج إدخاَل بياناٍت أصًلا.
#:
#: **وما لم يكن مقطوعًا به حسمه المالك (2026-09-18):**
#:
#: - ``REQPASS``: جواُز الوافد يصدر من سفارة بلده، والمعاملُة عندنا تحديُث
#:   بياناته على الإقامة — فالجهة «الإقامات والداخلية».
#: - ``leave`` (إذن مغادرة البلاد): «العمل والعمالة» — القوى العاملة عبر سهل.
#:
#: وما يُضاف بعدهما يُضاف بقرار: «غلاٌف لا يسمّي الجهَة أهوُن من غلاٍف
#: يسمّي جهًة خاطئة» (``_gov_cover_lines``).
GOV_COVER_CATEGORY: dict[str, str] = {
    "REQWP": "manpower",      # إذُن العمل — الهيئة العامة للقوى العاملة
    "REQCID": "civil_id",     # البطاقُة المدنية — الهيئة العامة للمعلومات المدنية
    "REQRESE": "residency",   # تجديُد الإقامة المبكر
    "REQRESN": "residency",   # تجديُد الإقامة العادي
    "REQPASS": "residency",   # تحديث الجواز على الإقامة — قرار المالك 2026-09-18
    "leave": "manpower",      # إذن مغادرة البلاد (القوى العاملة/سهل) — قرار المالك 2026-09-18
}
...
#: ``needs``:
#:
#: - ``template`` — لا قالب للمستند أصًلا: يلزم مستٌند يُصاغ ويُعتمد.
#: - ``profile_template`` — للمستند قوالب، **ولا قالب لصورته**. و``OD-018``
#:   مثاله: يغطّي أربع صور (استئذان · تصحيح حضور · إضافي · عودة) وقوالبه
#:   اثنان. وهذا التصنيف لم يكن في أول تقسيم لي — صنّفتُ الصور «ربًطا» لأن
#:   المستند له قالب، فقاس تصنيفي المستندَ لا صورته. والفرق عمٌل قائم أو
#:   ورقٌة تُصاغ.
#: - ``wiring`` — القالب موجود ويطابق الصورة، والربط وحده ناقص (قرار هندسي).
#: - ``signature_decision`` — إصداُره يحوّل الطلب إلى انتظار توقيٍع ورقّي
#:   لأن رايَة التوقيع مفعَّلٌة في نوعه. وهو تغيٌّر في المسار لا إصداُر
#:   ورقة، فيحتاج قراًرا.
#: - ``archive_privacy`` — (أُغلق) كان: **المستند سرٌّي والأرشيف لا يعرف
#:   السرّية**.
#:   إصداُره اليوم يضعه في ملف الموظف حيث يقرؤه كل من يحمل
#:   ``view_documents`` — ومنهم مدير الشركة. و``OD-010`` (نتيجة تظلّم)
#:   أوضُحها ضرًرا: إصداُره ينقض حجَب الشكوى عن المشكو منه.
#:
#:   وهذا التصنيف لم يكن في التقسيم الأول: ظُنّ العائق **صياغَة ورقة**،
#:   فإذا هو أن **مكاَن الورقة لا يحفظ سرَّها**. وقد بُنيت خصوصيُة
#:   الأرشيف فزال — ولم يبق سطٌر بهذا التصنيف.
OUTPUT_GAPS: dict[str, dict] = {
    # — يلزمها مستٌند يُصاغ ويُعتمد —
    "WF-014/OD-009": {"needs": "declaration",
                      "why": ("إقرار/ردّ الموظف — **يُنتَج فعًلا** تحت WF-015 "
                              "(REQWARN وREQVIO). فهذه ليست فجوَة عمٍل بل أثُر "
                              "قياس: WF-014 (إصدار الإنذار) يعلن مستنَد الردّ "
                              "بين مخرجاته، والردُّ مساٌر آخر. فال يُصاغ له قالٌب "
                              "— هو موجود."),},
...
def produced_outputs() -> dict[str, set[str]]:
    ...
    for rt in DEFAULT_REQUEST_TYPES:
        ...
        if not (rt.get("produces_document")
            ...
            continue
        # **والهويّة من ``canonical_od_for`` لا من القالب.**
        #
        # كانت تُشتقّ هنا من ``default_template_code`` وحده، وقد صارت
        # الهويّة تُحَل من السجلّ في P1-02. فبقي طريقان لسؤاٍل واحد:
        # المولّد يقول «هذا النوع يُنتج OD-022» وهذه تقول «لا يُنتج
        # شيًئا» — لأن النوع بلا قالب.
        #
        # وأثرُه أن سجلّ الثغرات يُبقي ثغرًة أُغلقت فعًلا، وحارُس «العذر
        # يسقط بزوال سببه» يوافقه لأنه يقرأ المقياس المعطوب نفسه. وهو
        # عين النمط الذي يُطارَد في هذه الجولة.
        od = canonical_od_for(rt["code"], rt.get("default_template_code"))
        if not od:
            continue
        ...
        # **ومخرُج المرحلة يُحتسب حيث يقع**: مرحلٌة شرطّية بالحمولة
        # مخرجُها مخرُج المسار المُرقَّى ال األصلي — فغالُف المتابعة مخرُج
        # ``WF-002`` (إجازة سفر) ال ``WF-001``.
        for (code, stage_kind), extra in STAGE_EXTRA_OUTPUTS.items():
            if code != rt["code"]:
                continue
            if not any(st.get("kind") == stage_kind for st in chain):
                continue
            conditional = any(st.get("kind") == stage_kind and st.get("when")
                              for st in chain)
            targets = [canonical]
            if conditional:
                targets = [promoted for base, _f, promoted in _PAYLOAD_PROMOTIONS
                           if base == canonical] or [canonical]
            for wf in targets:
                out.setdefault(wf, set()).add(extra)
    return out
...
#
# **العطل المقيس**: ``generate_document`` كان يشتقّ ``od_code`` من
# ``default_template_code`` وحده — أي أن القالب يحدّد هويّة المستند
# القانونية. والسجلّ يعلن لكل مسار مستنداته ولا يُقرأ.
#
# وأثره ظهر عند محاولة ربط العشرة المعلَّقة: **تسعٌة من عشرة قالُبها يشير
# إلى مستٍند غير الذي يعلنه مسارها** — وبعضها يشير إلى *تقرير* لا مستند:
#
#     REQCID  تحديث بطاقة مدنية  →  OD-006 «قرار إنذار»
#     REQPASS تحديث جواز         →  OD-007 «محضر تحقيق إداري»
#     REQDATA تعديل بيانات       →  OD-016 «شهادة إخلاء طرف»
#     REQATT  تصحيح حضور         →  RPT-002 (تقرير)
#
# ولم يمسك ذلك حارٌس لأن حارس P1-02 يمرّ على **منتجي المستندات** وحدهم،
# وهؤلاء ``produces_document=False``. فالمؤشّرات الخاطئة كانت مستورًة خلف
# أن النوع لا يُنتج شيًئا — ولو رُبطت كما هي لصدر «تحديث بطاقة مدنية»
# مختوًما «قرار إنذار».
#
# فصارت الهويّة من المسار: يعلن واحًدا فهو هو، ويعلن عدًدا فالقالب يختار
# بينها، ولا يعلن شيًئا فلا تُخمَّن.
# ==============================================================================

#: أنواٌع هويُّة مستندها من قالبها وحده — **لأن مسارها لا يعلن مستنًدا**.
#:
#: وهي مذكورٌة بأسمائها لا متروكٌة لسقوٍط عام: السكيل يشترط أن يكون غياب
#: الربط خطًأ صريًحا يسمّي المسار، لا صمًتا يُنتج مستنًدا بلا هويّة. وكلٌّ
#: منهما مسجٌَّل أصًلا في ``UNDECIDED`` بحارس الخريطة.
OD_FROM_TEMPLATE_ONLY: dict[str, str] = {
    "REQMIS": "«مهمة عمل خارجية» — مسارها WF-029 (تصنيف عام) لا يعلن أي OD.",
    "ADMLIC": "«تجديد مستند شركة» — كيانه الشركة لا الموظف، ولا OD معلٌَن لمساره.",
}


def canonical_od_for(code: str, template_code: str | None) -> str | None:
    """هويّة المستند الذي يُصدره هذا النوع — من السجلّ أوًلا.

    - المسار يعلن مستنًدا واحًدا → هو، ولو خالفه القالب.
    - يعلن عدًدا → القالب يختار بينها (ولا يخرج عنها).
    - لا يعلن شيًئا → القالب وحده، وللمذكورين في
      :data:`OD_FROM_TEMPLATE_ONLY` فقط. وغيرهم ``None`` كي يصرخ المولّد
      باسم المسار بدل أن يختم ورقًة بلا هويّة.
    """
    entry = LEGACY_REQUEST_ALIASES.get(code) or {}
    canonical = entry.get("canonical") if isinstance(entry, dict) else None
    declared = (CANONICAL_WORKFLOWS.get(canonical) or {}).get("od") or []
    producing = [od for od in declared
                 if (CANONICAL_DOCUMENTS.get(od) or {}).get("produces_pdf")]
    from_template = resolve_template(template_code) if template_code else None

    if from_template and from_template in producing:
        return from_template
    if len(producing) == 1:
        return producing[0]
    if not producing and code in OD_FROM_TEMPLATE_ONLY:
        return from_template
    return None
