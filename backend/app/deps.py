...
#:
#: **وكانت «القاعدةُ الواحدة» قاعدتين.** ``workflow.BLOCKED_EMPLOYEE_STATUSES``
#: كُتبت بالتعليل نفسه («كل مسار إنشاء يمرّ بها، فلا يبقى باب ثانٍ يُنسى»)
#: وبقائمةٍ أخرى: ``(terminated, archived, resigned)``. فالمستقيلُ ممنوعٌ من
#: فتح طلبٍ ومن البصم، ومسموحٌ له باستبدال توقيعه؛ **والمتقاعدُ لم تمنعه
#: أيٌّ منهما** — يبصم ويفتح طلبًا ويستبدل توقيعه. فصارت هنا قائمةً واحدة،
#: وهناك اسمًا لها لا نسخةً منها.
#:
