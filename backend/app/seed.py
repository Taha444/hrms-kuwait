...
DEFAULT_TEMPLATES = [
    ...
    ("HRMS-PR-002", "شهادة لمن يهمه الأمر", "To Whom It May Concern Certificate", "الشهادات والخطابات",
     _tpl("HRMS-PR-002", "شهادة لمن يهمه الأمر", "To Whom It May Concern Certificate",
          "إلى من يهمه الأمر: تشهد {{company_name}} بأن السيد/السيدة {{employee_name}} يعمل لديها بوظيفة {{job_title}} منذ {{hire_date}} وما زال على رأس عمله حتى تاريخ إصدار هذا الخطاب. صدرت هذه الإفادة بناءً على طلبه لتقديمها إلى {{target_entity}} لغرض {{purpose}}.",
    ...
    ("HRMS-PR-003", "شهادة خبرة", "Experience Certificate", "الشهادات والخطابات",
     _tpl("HRMS-PR-003", "شهادة خبرة", "Experience Certificate",
          "تشهد {{company_name}} بأن السيد/السيدة {{employee_name}} قد عمل لديها خلال الفترة من {{service_start}} إلى {{service_end}} بوظيفة {{job_title}}، وأدى المهام الموكلة إليه وفق سجلات الشركة. صدرت هذه الشهادة بناءً على طلبه.",
    ...
    ("HRMS-PR-004", "شهادة حالة وظيفية", "Employment Status Certificate", "الشهادات والخطابات",
     _tpl("HRMS-PR-004", "شهادة حالة وظيفية", "Employment Status Certificate",
          "تفيد {{company_name}} بأن بيانات الحالة الوظيفية للموظف/ة {{employee_name}} (رقم مدني {{civil_id}}) كما هي مبينة في هذا المستند حتى تاريخ إصداره، وتم استخراجها مباشرة من نظام الموارد البشرية.",
    ...
    ("HRMS-PR-005", "خطاب عدم ممانعة", "No Objection Certificate", "الشهادات والخطابات",
     _tpl("HRMS-PR-005", "خطاب عدم ممانعة", "No Objection Certificate",
          "تشهد {{company_name}} بأنها لا تمانع في {{purpose}} للموظف/ة {{employee_name}}، بشرط عدم تعارضه مع التزاماته الوظيفية وسياسات الشركة، ودون ترتيب أي التزام مالي أو قانوني إضافي على الشركة ما لم يذكر خلاف ذلك صراحة.",
    ...
    ("HRMS-PR-007", "شهادة مدة خدمة", "Length of Service Certificate", "الشهادات والخطابات",
     _tpl("HRMS-PR-007", "شهادة مدة خدمة", "Length of Service Certificate",
          "تشهد {{company_name}} بأن مدة خدمة الموظف/ة {{employee_name}} المحتسبة وفق سجلات الشركة تمتد من {{hire_date}} حتى {{as_of_date}}، بإجمالي {{service_duration}} من الخدمة الفعلية.",
    ...
    ("HRMS-PR-010", "خطاب موجه لجهة رسمية", "Official Entity Letter", "الشهادات والخطابات",
     _tpl("HRMS-PR-010", "خطاب موجه لجهة رسمية", "Official Entity Letter",
          "السادة {{target_entity}} المحترمين، بالإشارة إلى طلبكم بشأن {{subject}}، نفيدكم بأن بيانات الموظف/ة {{employee_name}} (رقم مدني {{civil_id}}، وظيفة {{job_title}}) صحيحة وفق سجلات {{company_name}} حتى تاريخ إصدار هذا الخطاب. وتفضلوا بقبول فائق الاحترام.",
    ...
    ("HRMS-PR-011", "خطاب عرض وظيفي", "Employment Offer Letter", "العقود والقرارات الوظيفية",
     _tpl("HRMS-PR-011", "خطاب عرض وظيفي", "Employment Offer Letter",
          "يسر {{company_name}} أن تقدم للسيد/السيدة {{candidate_name}} عرضًا للعمل بوظيفة {{job_title}} في {{location}} اعتبارًا من {{start_date}}، براتب أساسي {{basic_salary}} د.ك وبدلات {{allowances}} د.ك، وفق الشروط الموضحة أدناه. يصبح العرض نافذًا بعد توقيع الطرفين واستكمال المستندات والموافقات المطلوبة.",
          ...
          [("[____] /KWD إجمالي الراتب [____] /KWD الراتب الأساسي", "Basic Salary Total Package"), ("[DD/MM/YYYY] تاريخ المباشرة [ ] فترة التجربة", "Probation Start Date"), ("[DD/MM/YYYY] مدة صلاحية العرض [ ] ساعات العمل", "Working Hours Offer Validity")],
    ...
    ("HRMS-PR-018", "قرار ترقية", "Promotion Decision", "العقود والقرارات الوظيفية",
     _tpl("HRMS-PR-018", "قرار ترقية", "Promotion Decision",
          ...
          [("[ ] المسمى الجديد [ ] المسمى السابق", "Previous Title New Title"), ("[____] /KWD الراتب الجديد [____] /KWD الراتب السابق", "Previous Salary New Salary"), ("[DD/MM/YYYY] تاريخ النفاذ [ ] الدرجة المستوى", "Grade / Level Effective Date")],
          [("Prepared By", "إعداد"), ("Reviewed By", "مراجعة"), ("Approved By", "اعتماد")], has_ack=True)),
    ("HRMS-PR-019", "قرار تعديل راتب", "Salary Adjustment Decision", "العقود والقرارات الوظيفية",
     _tpl("HRMS-PR-019", "قرار تعديل راتب", "Salary Adjustment Decision",
          ...
          [("[____] الراتب الرسمي الجديد [____] الراتب الرسمي السابق", "Previous Official Salary New Official Salary"), ("[____] الراتب الفعلي الجديد [____] الراتب الفعلي السابق", "Previous Actual Salary New Actual Salary"), ("[MM/YYYY] شهر التطبيق [ ] سبب التعديل", "Reason Payroll Month")],
          [("Prepared By", "إعداد"), ("Reviewed By", "مراجعة"), ("Approved By", "اعتماد")], has_ack=True)),
    ("HRMS-PR-020", "قرار بدل أو مكافأة", "Allowance / Bonus Decision", "العقود والقرارات الوظيفية",
     _tpl("HRMS-PR-020", "قرار بدل أو مكافأة", "Allowance / Bonus Decision",
          ...
          [("[____] /KWD القيمة [Allowance/Bonus] النوع", "Type Amount"), ("[Payroll/Separate] طريقة الصرف [ ] الفترة", "Period Payment Method"), ("[Yes/No] خاضع لالستقطاع [ ] السبب", "Reason Subject to Deduction")],
          [("Prepared By", "إعداد"), ("Reviewed By", "مراجعة"), ("Approved By", "اعتماد")], has_ack=False)),
    ...
    ("HRMS-PR-029", "بيان رصيد الإجازات", "Leave Balance Statement", "الحضور والإجازات والرواتب",
     _tpl("HRMS-PR-029", "بيان رصيد الإجازات", "Leave Balance Statement",
          ...
          [("[____] /Days استحقاق السنة [____] /Days الرصيد المرحل", "Carried Forward Annual Entitlement"), ("[____] /Days المعلق [____] /Days المستخدم", "Used Pending"), ("[____] /Days الرصيد المتاح [____ /+] التعديالت يوم -", "Adjustments Available Balance")],
          [("Prepared By", "إعداد"), ("Reviewed By", "مراجعة"), ("Approved By", "اعتماد")], has_ack=False)),
    ...
    ("HRMS-PR-034", "تفويض تجديد إقامة", "Residency Renewal Authorization", "المعاملات الحكومية والمستندات",
     _tpl("HRMS-PR-034", "تفويض تجديد إقامة", "Residency Renewal Authorization",
          "تفوض {{company_name}} المندوب {{delegate_name}} باستكمال إجراءات تجديد إقامة الموظف/ة {{employee_name}} (رقم مدني {{civil_id}}، إقامة رقم {{residency_no}} المنتهية في {{expiry_date}}). نوع التجديد {{renewal_type}} وسببه {{reason}}، تحت ملف الشركة رقم {{company_file_no}}.",
    ...
    ("HRMS-PR-038", "التسوية النهائية ونهاية الخدمة", "Final Settlement & End of Service", "إنهاء الخدمة والتسليم",
     _tpl("HRMS-PR-038", "التسوية النهائية ونهاية الخدمة", "Final Settlement & End of Service",
          ...
          [("[____] راتب مستحق [____] مكافأة نهاية الخدمة", "End of Service Benefit Outstanding Salary"), ("[____] مستحقات أخرى [____] بدل إجازات", "Leave Encashment Other Entitlements"), ("[____] سلف متبقية [____] خصومات عهد", "Deductions / Assets Outstanding Advances"), ("[____] الصافي النهائي [____] إجمالي المستحق", "Gross Payable Net Settlement")],
...
def build_company(db, cfg) -> dict:
    ...
    admin_staff = [
        (manager, "مدير الشركة", 2500, "both", hq.id, -1200),
        (hr, "موظف موارد بشرية", 900, "both", hq.id, -1000),
        (deleg1, "مندوب أول", 550, "qr", hq.id, -800),
        (deleg2, "مندوب ثاني", 550, "qr", hq.id, -750),
        (sup1, f"مسؤول {cfg['branches'][0][0]}", 800, "both", branches[0].id, -900),
        (sup2, f"مسؤول {cfg['branches'][1][0]}", 800, "both", branches[1].id, -850),
        (accountant, "محاسب", 750, "both", hq.id, -500),
    ]
