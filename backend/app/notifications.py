...
def sla_scan(db: Session) -> dict:
    ...
    for task in open_tasks:
        ...
        # **والمهمُّة المُصعَّدة تحمل أثر تصعيدها.**
        #
        # ``escalated_at`` كان عموًدا لا يكتبه شيء: فمن يقرأ المهمة
        # الأصلية لا يعرف أنها صُعِّدت، والمسُح لا يملك علامًة غير سلسلٍة
        # نصّية. والعلامُة تُكتب هنا لأن التصعيد وقع هنا.
        #
        # أمّا ``escalation_task_id`` فعمٌود مبٌنّي على فرٍض لا يقع:
        # ``notify_roles`` تُنشئ مهمًة **لكل مستقبِل** (الشؤون ومدير
        # الشركة)، فمفتاٌح أجنبٌي واحد لا يحمل رابًطا متفرًّعا. والرابُط
        # القائم هو بادئُة ``sla_escalation:{task.id}:`` — تُقرأ بها
        # التصعيداُت كلُّها. فلا يُملأ عموٌد بنصف الحقيقة.
