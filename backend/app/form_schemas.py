...
SCHEMAS.update({
    ...
    "ADMWARN": {
        "fields": [
            ...
            _field("effective_date", "تاريخ السريان", "date", required=True),
            # **حقٌل إلزامٌي لا يقرؤه شيٌء بعد — قراٌر معلَّق.**
            #
            # قيس: لا موضَع في النظام يقرأ ``response_deadline`` — لا
            # تذكيٌر قبل انقضائها، ولا إخطاٌر عند انقضائها بلا ردّ. فموظف
            # الشؤون **يُلزَم** بإدخال تاريٍخ لا يفعل شيًئا.
            #
            # والطرُف القادر على الردّ موجوٌد ويعلم: الموظُف يُخطَر
            # بالإنذار (``NTF-071``)، و``REQWARN`` (الردّ)
            # ``visible_to_employee=True``. فالنقُص في المهلة وحدها.
            #
            # **ولم تُوصَل بقصد**: الإنذاُر مساٌر تأديبٌّي قانوني، ومهلُة
            # الردّ حٌق إجرائي — ووصُل تذكيٍر أو انقضاٍء يُنشئ سلوًكا
            # قانونًيا لم يُعتمَد. فيُنتظَر قراُر صاحبه بين ثالثة:
            # تُوصَل، أو يُرفَع إلزامُها، أو تبقى إلزاًما ورقًيا يُعرَف أنه
            # ورقّي. يحرسه ``test_zzz_warning_deadline_state.py``.
