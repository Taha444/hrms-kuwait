...
ROLE_DEFAULT_PERMS: dict[str, set[str]] = {
    ...
    # المالك: دور رقابي — متابعة الشركات/الفروع/الأداء/التقارير، ولا أعمال
    # تشغيلية يومية. يشمل الاطلاع على سجل التدقيق والرواتب (رقابة/حوكمة).
    #
    # **و``manage_company`` استثناٌء بقرار المالك (2026-09-11).** كان
    # ``PUT /companies/{id}`` محروًسا بـ``require_super_admin`` وحده،
    # وقاعدة المالك المعلَنة تمنع منح ``super_admin`` لأيّ شخص. فكانت
    # النتيجة أن **بيانات الشركة لا يعدّلها أحٌد في الإنتاج** — ومنها رقم
    # الملف الذي أُضيف مع حزمة البيانات. أي بياٌن يُقرأ ولا يُصحَّح.
    #
    # وهذا يضيّق FIX-010 ولا يلغيه: التعديل مقصوٌر على بيانات الشركة
    # نفسها، وإنشاء الشركة وتعطيلها يبقيان للإدارة العليا بقرار المالك
    # في الرسالة نفسها.
    "company_owner": {"view_employee", "view_reports", "export_reports", "view_tasks",
                      ...
                      "view_attendance", "manage_company"},
    ...
    # لا إعدادات نظام/قوالب/تدقيق/نقل بين الشركات (الإدارة العليا).
    #
    # **و``manage_company`` بقرار المالك (2026-09-11)**: يعدّل بيانات
    # **شركته** — والنطاق محروٌس في المسار لا هنا، فالصلاحية لا تحمل
    # نطاًقا. وإنشاء الشركة وتعطيلها يبقيان للإدارة العليا.
...
# **قرار المالك (2026-09-24): صاحب الشركات وحده.** ثم رُفع الإلزام عن مدير الشركة في اليوم نفسه (يفعّله بمحض
# إرادته من صفحة «التحقق الثنائي») ليعمل على النظام؛ وكان أولًا مدير الشركة وصاحبها، وقبلهما خمسة
# (وHR والمندوب والإدارة العليا)، وضاق النطاق إلى من تكفي كلمةُ مرورِه وحدها لاختراق
# شركةٍ كاملة. ومن عداهم يبقى التفعيل لهم **اختياريًا** (صفحة «التحقق الثنائي») ولا
# يُحصَرون فيها ولا يُمنعون.
TWOFA_REQUIRED_ROLES = {"company_owner"}
